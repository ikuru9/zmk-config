...
if [ -n "${ZMK_SRC_DIR:-}" ]; then
    west_workspace=$(dirname "$(dirname "$(realpath "$ZMK_SRC_DIR")")")
fi

test_module_options=""
if [ -e "$west_workspace/zmk-keypeek-layer-notifier/Kconfig" ]; then
    test_module_options="-DCONFIG_ZMK_KEYPEEK_LAYER_NOTIFIER=n -DCONFIG_RAW_HID=n"
fi
...
build_cmd="west zephyr-export >/dev/null && west build ${ZMK_SRC_DIR:+-s $ZMK_SRC_DIR} -d ${ZMK_BUILD_DIR}/tests/$testcase \
    -b native_sim//zmk_test_mock -p -- -DCONFIG_ASSERT=y $test_module_options -DZMK_CONFIG=$(realpath "$path") \
...
mkdir -p "$(dirname "$build_log_tmp")" "$(dirname "$build_log")"
(cd "$west_workspace" && sh -c "$build_cmd") >"$build_log_tmp" 2>&1
build_exit_code=$?

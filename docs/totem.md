...
- In dongle topology, the selected `totem_dongle`, `totem_zdd_dongle`, or `totem_prospector_dongle` target is the split BLE hub endpoint and both keyboard halves are peripherals.
...
# build ZDD dongle split targets
docker compose run --rm zmk-build-stable --artifact-names totem_left_w_dongle,totem_right,totem_zdd_dongle,reset_nice_nano_v2,reset_seeeduino_xiao_ble

# build Prospector dongle split targets
docker compose run --rm zmk-build-stable --artifact-names totem_prospector_dongle,totem_left_w_dongle,totem_right,reset_seeeduino_xiao_ble
```
...
- `totem_prospector_dongle.uf2`
- `reset_seeeduino_xiao_ble.uf2`
- `reset_nice_nano_v2.uf2`

### Flashing

Assumes XIAO BLE for halves, nice!nano v2 for the standard/ZDD dongle, and XIAO BLE for the Prospector dongle, all with UF2 bootloader support.

Direct split (`totem_left` + `totem_right`):

1. Connect a half to the PC via USB and enter bootloader mode.
2. Optional: flash `reset_seeeduino_xiao_ble.uf2` if you need to clear settings/bonds.
3. Flash `totem_left.uf2` to the left half.
4. Flash `totem_right.uf2` to the right half.
5. Power-cycle both halves.

ZDD dongle split (`totem_zdd_dongle` + `totem_left_w_dongle` + `totem_right`), clean start recommended:

1. Flash reset firmware first:
   - `reset_nice_nano_v2.uf2` to the dongle.
   - `reset_seeeduino_xiao_ble.uf2` to both keyboard halves.
...
3. Remove the old dongle bond on the host, then pair the host to the dongle again.
4. Power-cycle the dongle and both halves if split links do not reconnect immediately.

Prospector dongle split (`totem_prospector_dongle` + `totem_left_w_dongle` + `totem_right`), clean start recommended:

1. Flash `reset_seeeduino_xiao_ble.uf2` to the Prospector dongle and both keyboard halves to clear stale bonds.
2. Flash runtime firmware:
   - `totem_prospector_dongle.uf2` to the Prospector dongle.
   - `totem_left_w_dongle.uf2` to the left half.
   - `totem_right.uf2` to the right half.
3. Remove the old dongle bond on the host, then pair the host to the Prospector dongle again.
4. Power-cycle all three devices and confirm USB input, both split links, and Prospector screen/status updates.
